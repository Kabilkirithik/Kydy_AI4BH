...
async function fetchCourseProgress() {
  try {
    ...
    return [
      { title: 'Web Development',  icon: '⚡', color: '#3B82F6', progress: 68, lessons: 17, lessonsTotal: 25, tag: 'In Progress'  },
      { title: 'Machine Learning', icon: '🤖', color: '#3B82F6', progress: 35, lessons: 8,  lessonsTotal: 23, tag: 'In Progress'  },
      { title: 'UI/UX Design',     icon: '🎨', color: '#8B5CF6', progress: 82, lessons: 21, lessonsTotal: 26, tag: 'Almost Done'  },
    ]
  }
}
...
function Orb({ style, color = '#3B82F6', size = 400 }: { style?: CSSProperties; color?: string; size?: number }) {
  return (
    <div style={{
...
function StatCard({ icon, label, value, sub, color }: StatCardData) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 28, fontWeight: 900, color: '#0F172A', fontFamily: "'Poppins',sans-serif", marginBottom: 3 }}>{value}</div>
      <div style={{ fontSize: 11, fontWeight: 700, color: '#64748B', fontFamily: "'Poppins',sans-serif", letterSpacing: 0.5, marginBottom: 4 }}>{label}</div>
...
function DashboardContent({ onNav }: { onNav: (id: string) => void }) {
  const [stats, setStats] = useState<StatCardData[]>([
    { icon: '⏱', label: 'Study Time',  value: '24h', sub: '+3h this week',    color: '#3B82F6' },
    { icon: '🔥', label: 'Streak',      value: '12',  sub: 'Days in a row',    color: '#f59e0b' },
    { icon: '📖', label: 'In Progress', value: '3',   sub: 'Active courses',   color: '#3B82F6' },
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        ...
        setStats([
          { icon: '⏱', label: 'Study Time',  value: statsData.study_time, sub: statsData.study_time_sub, color: '#3B82F6' },
          { icon: '🔥', label: 'Streak',      value: statsData.streak,     sub: statsData.streak_sub,     color: '#f59e0b' },
          { icon: '📖', label: 'In Progress', value: statsData.in_progress, sub: statsData.in_progress_sub, color: '#3B82F6' },
  ...
  const activity = [
    { day: 'MON', hrs: 2.5, color: '#3B82F6' },
    { day: 'TUE', hrs: 1.0, color: '#3B82F6' },
    { day: 'WED', hrs: 3.5, color: '#3B82F6' },
    { day: 'THU', hrs: 2.0, color: '#3B82F6' },
    { day: 'FRI', hrs: 4.0, color: '#8B5CF6' },
    { day: 'SAT', hrs: 1.5, color: '#3B82F6' },
    { day: 'SUN', hrs: 3.0, color: '#3B82F6' },
  ]

  return (
    <main style={{ flex: 1, padding: '36px 40px', overflowY: 'auto', position: 'relative', background: '#F8FAFC' }}>
      <Orb style={{ top: '-5%', right: '5%' }}   color="#8B5CF6" size={500} />
      <Orb style={{ bottom: '10%', left: '10%' }} color="#3B82F6" size={350} />

      {/* ── Header ── */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 36, position: 'relative', zIndex: 2 }}>
        <div>
          <div style={{ color: '#3B82F6', fontSize: 10, fontFamily: "'Poppins',sans-serif", letterSpacing: 3, marginBottom: 5 }}>OVERVIEW</div>
          <h1 style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 900, fontSize: '1.75rem', color: '#0F172A', letterSpacing: -1 }}>DASHBOARD</h1>
        </div>
      </div>
      ...
      <div style={{ position: 'relative', zIndex: 2, marginBottom: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
          <div>
            <div style={{ color: '#3B82F6', fontSize: 10, fontFamily: "'Poppins',sans-serif", letterSpacing: 3, marginBottom: 4 }}>CURRENTLY STUDYING</div>
            <h2 style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 800, fontSize: '1.05rem', color: '#0F172A', letterSpacing: -0.5 }}>MY COURSES</h2>
          </div>
        </div>
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                    <span style={{ color: '#0F172A', fontSize: 13, fontWeight: 700, fontFamily: "'Poppins',sans-serif", letterSpacing: 0.3 }}>{c.title}</span>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6 }}>
                    <span style={{ color: '#64748B', fontSize: 11, fontFamily: "'Inter',sans-serif" }}>{c.lessons}/{c.lessonsTotal} lessons</span>
                    <span style={{ color: c.color, fontSize: 11, fontFamily: "'Poppins',sans-serif", fontWeight: 700 }}>{c.progress}%</span>
                  </div>
                </div>
                <LGBtn variant="enroll" onClick={() => handleContinueCourse(c.title)} style={{ padding: '8px 16px', borderRadius: 9, flexShrink: 0, fontFamily: "'Poppins',sans-serif", fontSize: 9, fontWeight: 700, letterSpacing: 1, color: '#0F172A' }}>
                  CONTINUE
                </LGBtn>
              </div>
            ))
      ...
      <div style={{ position: 'relative', zIndex: 2 }}>
        <div style={{ color: '#3B82F6', fontSize: 10, fontFamily: "'Poppins',sans-serif", letterSpacing: 3, marginBottom: 4 }}>THIS WEEK</div>
        <h2 style={{ fontFamily: "'Poppins',sans-serif", fontWeight: 800, fontSize: '1.05rem', color: '#0F172A', letterSpacing: -0.5, marginBottom: 16 }}>ACTIVITY</h2>
        <div style={{ background: '#ffffff', border: '1px solid #E2E8F0', borderRadius: 18, padding: '22px 28px' }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, height: 100 }}>
            {activity.map(({ day, hrs, color }) => (
              <div key={day} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5, height: '100%', justifyContent: 'flex-end' }}>
                <span style={{ color: '#64748B', fontSize: 9, fontFamily: 'monospace' }}>{hrs}h</span>
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .lg-primary { background: linear-gradient(145deg,rgba(59,130,246,0.55),rgba(139,92,246,0.38),rgba(245,158,11,0.45)); border: 1px solid rgba(196,149,255,0.45) !important; box-shadow: 0 0 0 1px rgba(255,255,255,0.06) inset, 0 1px 0 rgba(255,255,255,0.2) inset, 0 -1px 0 rgba(0,0,0,0.3) inset, 0 0 28px rgba(59,130,246,0.35), 0 4px 18px rgba(0,0,0,0.4); }
        .lg-primary:hover { box-shadow: 0 0 0 1px rgba(255,255,255,0.08) inset, 0 1px 0 rgba(255,255,255,0.24) inset, 0 -1px 0 rgba(0,0,0,0.3) inset, 0 0 48px rgba(59,130,246,0.6), 0 6px 26px rgba(0,0,0,0.45); }
        ...
        .lg-enroll { background: linear-gradient(145deg,rgba(59,130,246,0.45),rgba(139,92,246,0.32)); border: 1px solid rgba(139,92,246,0.4) !important; box-shadow: 0 1px 0 rgba(255,255,255,0.13) inset, 0 -1px 0 rgba(0,0,0,0.2) inset, 0 0 16px rgba(59,130,246,0.2); }
        .lg-enroll:hover { box-shadow: 0 1px 0 rgba(255,255,255,0.2) inset, 0 -1px 0 rgba(0,0,0,0.2) inset, 0 0 32px rgba(59,130,246,0.46); }
      `}</style>

      <ClickSpark sparkColor="#3B82F6" sparkSize={10} sparkRadius={18} sparkCount={10} duration={450} extraScale={1.1}>
        <div style={{ display: 'flex', height: '100vh', background: '#F8FAFC', color: '#0F172A', overflow: 'hidden', position: 'relative' }}>
          <Orb style={{ top: '20%', left: '25%' }} color="#3B82F6" size={700} />
